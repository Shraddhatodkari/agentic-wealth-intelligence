...
> "Nimbus Dynamics, Inc." — a fictional company created for
...
non-compliance) — computed deterministically from each company's
...
Robotics), three different companies top three different rankings —
...
| **Google Gemini 3.6 Flash / Ollama** | LLM | `LLM_MODE=mock\|live\|ollama` — Gemini for cloud, Ollama for a fully local model with no API key |
...
| **pytest / pytest-cov / fakeredis** | Testing | 125 tests, 95% coverage, fully offline via mocks (LLM, HTTP, Redis, Celery, Ollama) |
...
4. **Tests + coverage** — `pytest --cov=src --cov-report=xml` (currently
   95%, 125 tests — including validation of every Grafana dashboard
...
├── tests/                        # 125 tests, all passing offline, 95% coverage
├── data/
...
pytest tests/ -v                # 125 tests, all pass with zero external services
...
and six tabs: **🔍 Analyze** (with live confidence/approval-status
badges), **📈 Compare Years**, **🏢 Portfolio** (cross-company rankings +
chart), **🧑‍⚖️ Pending Review** (the human reviewer queue — approve/reject
with an editable recommendation), **✅ Evaluate**, **🗂️ Report History** —
