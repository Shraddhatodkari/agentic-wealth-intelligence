...
def _parse_revenue_evidence(source_text: str) -> list[RevenueSignal]:
    """Extract only explicit revenue/net-sales changes.

    Conservative and company-agnostic:
    - requires revenue/sales language
    - requires an explicit percentage
    - avoids arbitrary substring capture
    - preserves sign from increase/decrease language
    """

    signals: list[RevenueSignal] = []

    # Normalize SEC HTML entities and whitespace.
    text = re.sub(r"&#\d+;|&[A-Za-z]+;", " ", source_text)
    text = re.sub(r"\s+", " ", text)

    patterns = [
        # Example:
        # Americas net sales increased ... 7%
        re.compile(
            r"\b(?P<segment>"
            r"(?:[A-Z][A-Za-z0-9&,'()/.-]*(?:\s+[A-Z][A-Za-z0-9&,'()/.-]*){0,7})"
            r")\s+"
            r"(?:net\s+sales|revenue|revenues|sales)"
            r"\s+"
            r"(?:"
            r"(?:increased|increases|grew|growth|rose|risen|declined|decreased|decreases|fell|falling)"
            r"|"
            r"(?:were|was)\s+(?:up|down)"
            r")"
            r".{0,100}?"
            r"(?P<pct>[0-9]+(?:\.[0-9]+)?)\s*%",
            re.IGNORECASE,
        ...
        # Example:
        # net sales in Europe increased ... 10%
        re.compile(
            r"(?:net\s+sales|revenue|revenues|sales)"
            ...
            r")"
            r".{0,100}?"
            r"(?:increased|increases|grew|rose|declined|decreased|fell|"
            r"were\s+up|were\s+down)"
            r".{0,100}?"
            r"(?P<pct>[0-9]+(?:\.[0-9]+)?)\s*%",
            re.IGNORECASE,
        ),
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            ...
            # Reject obvious sentence fragments / extraction artifacts.
            bad_fragments = {
                "the",
                "a",
                "an",
            ...
            if segment.lower() in bad_fragments:
                continue

            # Reject segments that contain too much prose.
            if len(segment) > 80:
                continue

            pct = float(match.group("pct"))

            # Determine direction from nearby evidence.
