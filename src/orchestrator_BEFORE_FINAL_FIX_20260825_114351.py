...
class WealthIntelligencePipeline:
    ...
    def _node_extract(self, state: PipelineState) -> PipelineState:
        ...
        with tracer.start_as_current_span("agent.extraction") as span:
            ...
            elapsed_ms = (time.perf_counter() - start) * 1000
            span.set_attribute("revenue_signal_count", len(result.revenue_signals))
            span.set_attribute("legal_risk_count", len(result.legal_risks))
            span.set_attribute("duration_ms", elapsed_ms)
        return {
            **state,
            "extraction": result,
    ...
    def run(self, filing_path: str, company: str, fiscal_year: str) -> PipelineState:
