...
def _parse_revenue_evidence(source_text: str) -> list[RevenueSignal]:
    """Company-agnostic extraction of explicit revenue/net-sales changes.

    Supports common 10-K sentence structures while remaining conservative:
    - requires revenue/sales terminology
    - requires an explicit percentage
    - derives direction from explicit increase/decrease language
    - never invents percentages
    - preserves the source sign
    """
    ...
    patterns = [
        # Segment + sales/revenue + direction + percentage
        re.compile(
            r"\b(?P<segment>[A-Za-z][A-Za-z0-9&,'()/.\- ]{1,70}?)\s+"
            r"(?P<metric>net\s+sales|sales|revenue|revenues)\s+"
            r"(?P<direction>increased|increases|grew|growth|rose|risen|"
            r"declined|decreased|decreases|fell|falling|"
            r"were\s+up|were\s+down|was\s+up|was\s+down)"
            r".{0,180}?"
            r"(?P<pct>[0-9]+(?:\.[0-9]+)?)\s*%",
            re.IGNORECASE,
        ),
        # Sales/revenue + direction + percentage + geographic/segment context
        re.compile(
            r"(?P<metric>net\s+sales|sales|revenue|revenues)\s+"
            r"(?P<direction>increased|increases|grew|growth|rose|risen|"
            r"declined|decreased|decreases|fell|falling|"
            ...
            r".{0,140}?"
            r"(?P<pct>[0-9]+(?:\.[0-9]+)?)\s*%"
            r"(?:.{0,80}?\b(?:in|for|from|within)\s+"
            r"(?P<segment>[A-Za-z][A-Za-z0-9&,'()/.\- ]{1,60}))?",
            re.IGNORECASE,
        ),
        # "sales in Europe increased 8%"
        re.compile(
            r"(?P<metric>net\s+sales|sales|revenue|revenues)\s+"
            r"(?:in|for|from|within)\s+"
            r"(?P<segment>[A-Za-z][A-Za-z0-9&,'()/.\- ]{1,60}?)\s+"
            r"(?P<direction>increased|increases|grew|rose|risen|"
            r"declined|decreased|decreases|fell|falling|"
            r"were\s+up|were\s+down|was\s+up|was\s+down)"
            r".{0,140}?"
            r"(?P<pct>[0-9]+(?:\.[0-9]+)?)\s*%",
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            segment = (match.groupdict().get("segment") or "").strip()
            ...
            # Avoid swallowing entire sentences.
            if len(segment) > 80:
            ...
            negative = bool(
                re.search(
                    r"\b(decreas|declin|fell|falling|down)\w*\b",
                    direction,
                    re.IGNORECASE,
                )
            )
            ...
            trend = "decline" if negative else "growth"
