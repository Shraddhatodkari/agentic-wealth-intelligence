...
- data_available=false means the metric was NOT reliably extracted.
- Never describe missing data as 0%, zero risk, zero exposure, or no growth.
- Do not use a company with data_available=false as evidence for a
  portfolio trend or comparison.
- Explicitly say "data unavailable" when a company's metric is missing.
- Only make growth, risk, or debt comparisons using companies with
  data_available=true for that dimension.
"""
...
def _rank(
    ...
    # Companies with real data rank first. Missing data must never be
    # interpreted as a genuine numeric zero.
    scored.sort(
        key=lambda triple: (
            not triple[2],
            -triple[1] if descending else triple[1],
        )
    )
    return [
        CompanyRank(
