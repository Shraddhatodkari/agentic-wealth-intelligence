...
async def compare(request: Request, body: CompareRequest, api_key: str = Depends(require_role("analyst"))):
    ...
    if settings.llm_mode == "mock":
        report = WealthIntelligencePipeline(llm=llm).run_yoy_comparison(
            prior_filing_path=body.prior_filing_path,
            current_filing_path=body.current_filing_path,
            company=body.company,
            prior_fiscal_year=body.prior_fiscal_year,
...
async def evaluate(request: Request, api_key: str = Depends(require_role("analyst"))):
    ...
    base = Path("data/sample_filings")
    ingestion = IngestionAgent()
    extraction_agent = ExtractionAgent(LLMClient(mode=settings.llm_mode))
    extractions_by_filing = {}
    for path in base.glob("*.txt"):
        chunks = ingestion.chunk(
            ingestion.load_local_filing(str(path)),
            section="10-K",
        )
        if settings.llm_mode == "mock":
            if path.name == "nimbus_dynamics_10k_fy2024.txt":
                extraction = ExtractionResult.model_validate(MOCK_EXTRACTION_FY2024)
            elif path.name == "nimbus_dynamics_10k_fy2025.txt":
        ...
        extractions_by_filing[path.name] = extraction
...
async def portfolio_analyze(
    ...
    if settings.llm_mode == "mock":
        report = portfolio_agent.analyze_portfolio(
            extractions,
            mock_response={"sector_narrative": MOCK_PORTFOLIO_NARRATIVE},
        )
    else:
